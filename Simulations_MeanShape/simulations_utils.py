...
def compute_all_means_known_param(pop_x_scale, pop_Z, pop_theta_func, h_deriv_bounds, h_bounds, lbda_bounds, nb_basis, pop_arclgth, n_call_bayopt=20, sigma=0.0):
    ...
    res_mean_arithm = collections.namedtuple('res_mean_arithm', ['mu', 'mu_s', 'mu_Z', 'mu_theta'])
    out_arithm = res_mean_arithm(mu_arithm, mu_s_arithm, mu_Z_arithm, mu_theta_arithm)
    ...
    res_mean_srvf = collections.namedtuple('res_mean_srvf', ['mu', 'mu_s', 'mu_Z', 'mu_theta'])
    out_srvf = res_mean_srvf(mu_srvf, mu_s_srvf, mu_Z_srvf, mu_theta_srvf)
...
def compute_all_means(pop_x, h_deriv_bounds, h_bounds, lbda_bounds, nb_basis, pop_arclgth=None, pop_X=None, n_call_bayopt=20, sigma=0.0):
    ...
    res_pop = collections.namedtuple('res_pop', ['mu_Z0', 'pop_theta', 'pop_theta_coefs', 'pop_Z', 'pop_X', 'pop_x_scale', 'pop_arclgth', 'pop_L'])
    out_pop = res_pop(mu_Z0, pop_theta, pop_theta_coefs, pop_Z, pop_X, pop_x_scale, pop_arclgth, pop_L)
...
def compute_SRC_FC_StatMeans(pop_Q, pop_theta_coefs, pop_arclgth, mu_Z0, h_bounds, lbda_bounds, nb_basis, n_call_bayopt=20, sigma=0.0):
    ...
    mu_Z_V1 = solve_FrenetSerret_ODE_SE(lambda s: np.squeeze(Bspline_decom.basis_fct(s).T @ coefs_opt), time, Z0=mu_Z0)
    mu_V1 = mu_Z_V1[:,:3,3]
    ...
    mu_Z_V2 = solve_FrenetSerret_ODE_SE(lambda s: np.squeeze(Bspline_decom.basis_fct(s).T @ coefs_opt), time, Z0=mu_Z0)
    mu_V2 = mu_Z_V2[:,:3,3]
    ...
    mu_Z_V3 = solve_FrenetSerret_ODE_SE(lambda s: np.squeeze(Bspline_decom.basis_fct(s).T @ coefs_opt), time, Z0=mu_Z0)
    mu_V3 = mu_Z_V3[:,:3,3]
