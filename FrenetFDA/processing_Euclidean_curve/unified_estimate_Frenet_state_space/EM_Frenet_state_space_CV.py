...
class FrenetStateSpaceCV_byit:
    ...
    def expectation_maximization(self, tol, max_iter, nb_basis, regularization_parameter_list = [], init_params = None, order=4, method='approx', model_Sigma='scalar', score_lambda='MSE_YX', verbose=False, n_splits_CV=10):
        ...
        if N_param_smoothing==0:
            ...
            penalization = True
        self.Bspline_decomp = VectorBSplineSmoothing(self.n-1, nb_basis, domain_range=self.domain_range, order=order, penalization=penalization)

        Y_train, grid_train, Y_test, grid_test = self.CV_split_Y(n_splits_CV)
        ...
        while k < max_iter and rel_error > tol:
            ...
            self.iteration_EM(Y_train, grid_train, Y_test, grid_test, regularization_parameter_list, method, model_Sigma, score_lambda)
    ...
    def iteration_EM(self, Y_train, grid_train, Y_test, grid_test, regularization_parameter_list, method='approx', model_Sigma='scalar', score_lambda='MSE_YX'):
        n_split = len(Y_train)
