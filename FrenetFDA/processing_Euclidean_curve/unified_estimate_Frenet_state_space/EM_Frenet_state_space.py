...
class FrenetStateSpaceCV_global:
    ...
    def E_step(self):
        ...
        self.P_full = kalman_filter.P_full_mat
    ...
    def __approx_distribution_r(self, method='monte_carlo', N_rand=500):
        ...
        elif method=='monte_carlo_GP':
            r = np.zeros((self.N, self.dim_g))
            cov_r = np.zeros((self.N, self.dim_g, self.dim_g))
            set_rand_xi = np.random.multivariate_normal(np.zeros((self.N+1)*self.dim_g), self.P_full, size=N_rand)
            set_rand_xi = np.reshape(set_rand_xi, (N_rand, self.N+1,self.dim_g))
            print(set_rand_xi.shape)
            rand_obs = np.zeros((N_rand, self.N, 2))
            for i in range(self.N):
                for j in range(N_rand):
                    rand_obs[j,i] = -(1/self.u[i])*self.L.T @ SE3.log(SE3.exp(set_rand_xi[j,i+1])@np.linalg.inv(self.Z[i+1])@self.Z[i]@SE3.exp(-set_rand_xi[j,i]))
            r = np.mean(rand_obs, axis=0)
            rand_obs_cov = np.zeros((N_rand, 2*self.N, 2*self.N))
            for j in range(N_rand):
                diff = np.reshape(rand_obs[j]-r, (2*self.N,))
                rand_obs_cov[j] = diff[:,np.newaxis] @ diff[np.newaxis,:]
            print(rand_obs_cov.shape)
            cov_r = np.mean(rand_obs_cov, axis=0)
            print(cov_r.shape)
            self.r_tilde = r
            self.cov_r_tilde = np.reshape(cov_r, (self.N,2,self.N,2))
            self.cov_r_tilde = np.moveaxis(self.cov_r_tilde, (1,2), (2,1))

            # for i in range(self.N):
            #     rand_obs = np.zeros((N_rand, self.dim_g))
            #     for j in range(N_rand):
            #         rand_obs[j] = -(1/self.u[i])*SE3.log(SE3.exp(set_rand_xi[j,i+1])@np.linalg.inv(self.Z[i+1])@self.Z[i]@SE3.exp(-set_rand_xi[j,i]))
            #         r[i] += rand_obs[j]
            #     r[i] = r[i]/N_rand
            #     for j in range(N_rand):
            #         cov_r[i] += (rand_obs[j] - r[i])[:,np.newaxis] @ (rand_obs[j] - r[i])[:,np.newaxis].T
...
def bayesian_CV_optimization_regularization_parameter(n_CV, n_call_bayopt, lambda_bounds, grid_obs, Y_obs, tol, max_iter, nb_basis, init_params = None, order=4, method='approx', verbose=False, knots=None):
    ...
    lbda_opt = 10 ** np.array([res_bayopt.x[0], res_bayopt.x[1]])
