...
print("n_tot_curves", n_tot_curves)

mat_SRC_scaled_1 = np.zeros((n_tot_curves, n_tot_curves))
mat_SRC_scaled_500 = np.zeros((n_tot_curves, n_tot_curves))
...
for i in range(n_tot_curves):
    ...
    for j in range(i+1, n_tot_curves):
        ...
        mat_SRC_scaled_1[i,j] = SRC(3).dist_bis(fi, fj, full_grid_arc_s_reshape[i], full_grid_arc_s_reshape[j], grid_time, lam=1)
        mat_SRC_scaled_500[i,j] = SRC(3).dist_bis(fi, fj, full_grid_arc_s_reshape[i], full_grid_arc_s_reshape[j], grid_time, lam=500)
...
mat_SRC_scaled_1 = mat_SRC_scaled_1 + mat_SRC_scaled_1.T
mat_SRC_scaled_500 = mat_SRC_scaled_500 + mat_SRC_scaled_500.T
...
np.save("results/mat_SRC_scaled_1_full", mat_SRC_scaled_1, allow_pickle=True)
np.save("results/mat_SRC_scaled_500_full", mat_SRC_scaled_500, allow_pickle=True)
np.save("results/mat_SRVF_full", mat_SRVF, allow_pickle=True)
np.save("results/mat_FC_scaled_full", mat_FC_scaled, allow_pickle=True)
